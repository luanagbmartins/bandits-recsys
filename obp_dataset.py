...
class MovieLensDataset(BaseRealBanditDataset):
    ...
    def pre_process(self) -> None:
        ...
        # self.data = self.data[self.data["rating"] >= 4]
        # df_list = []
        # for g, df_g in self.data.groupby("user_id"):
        #     list_of_indexes = []
        ...
        # self.data["movie_id_history"] = pd.concat(df_list)
        # self.data = self.data.fillna(method="ffill")

        self.context = np.concatenate(
            (
                self.user_embeddings[self.data["user_id"].values].cpu().numpy(),
                self.item_embeddings[self.data["movie_id"].values].cpu().numpy(),
            ),
            axis=1,
        )
        self.action_context = (
